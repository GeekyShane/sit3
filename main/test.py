...
def add_test_args(parser):
    ...
    files.add_argument('--model_dir', type=str, default='../modelx/',
    ...
    files.add_argument('--dev_src', nargs='+', type=str, default=['test.token.code'],
    ...
    bsearch.add_argument('--beam_size', type=int, default=5,
...
def set_defaults(args):
    ...
    if num_dataset > 1:
        ...
        if len(args.dev_guid) == 1:
            args.dev_guid = args.dev_guid * num_dataset
    ...
    for i in range(num_dataset):
        ...
        if args.use_code_type:
            ...
            dev_src_tag = None
        if args.use_code_struc:
            dev_guid = os.path.join(data_dir, args.dev_guid[i])
            if not os.path.isfile(dev_guid):
                raise IOError('No such file: %s' % dev_guid)
        else:
            dev_guid = None
...
def prepare_batch(batch, model):
    ...
    if args.cuda:
        ...
        if code_struc_rep is not None:
            code_struc_rep = code_struc_rep.cuda(non_blocking=True)
